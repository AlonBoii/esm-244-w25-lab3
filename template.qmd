...
4. Data split / into testing and training
...
6. Finalize the model (aka train it on whole dataset)
...
surv_df <- t_df |>
  mutate(survived = factor(survived), ### categorical outcome variables need to be factors
         pclass = factor(pclass)) |> ### turn some predictors to factors
  select(-cabin, -ticket)
...
lasso_rec<-recipe(survived~.,data=surv_train_df) %>%
  update_role(passenger_id, new_role = "ID") %>% ### passenger ID is just a tag, not a predictor or dependent variable
  step_rm(name,age) %>% ### don't want to consider name and age because of so much NAs
  step_unknown(all_nominal(),-all_outcomes()) |> ### if you see an NA make a new factor class called "uknown"
  step_dummy(all_nominal(),-all_outcomes()) |> ### all category variables are recognized as dummy variables
  step_zv(all_numeric(),-all_outcomes()) |> ### any variables with zero variance get rid of them
  step_normalize(all_numeric(),-all_outcomes()) ### all numeric data needs to be normalized
...
Lasso: takes all possible coefficient and says which ones do I need and which ones do I not need. Zero means those coefficients
don't matter. 
